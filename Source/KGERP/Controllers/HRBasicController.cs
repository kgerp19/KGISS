...
{
    [CheckSession]
    public class HRBasicController : Controller
    {

        private HttpContext httpContext;
        private readonly HRBasicService _service;
        ...
        [HttpPost]
        public async Task<ActionResult> Grade(VMGrade model)
        {

            if (model.ActionEum == ActionEnum.Add)
            {
                //Add 
                await _service.GradeAdd(model);
            }
            else if (model.ActionEum == ActionEnum.Edit)
            {
                //Edit
                await _service.GradeEdit(model);
            }
            else if (model.ActionEum == ActionEnum.Delete)
            {
                //Delete
                await _service.GradeDelete(model.ID);
            }
            else
            {
                return RedirectToAction("Error");
            }
            return RedirectToAction(nameof(Grade), new { companyId = model.CompanyFK });
        }
        ...
        [HttpPost]
        public async Task<ActionResult> Department(VMDepartment model)
        {

            if (model.ActionEum == ActionEnum.Add)
            {
                //Add 
                await _service.DepartmentAdd(model);
            }
            else if (model.ActionEum == ActionEnum.Edit)
            {
                //Edit
                await _service.DepartmentEdit(model);
            }
            else if (model.ActionEum == ActionEnum.Delete)
            {
                //Delete
                await _service.DepartmentDelete(model.ID);
            }
            else
            {
                return RedirectToAction("Error");
            }
            return RedirectToAction(nameof(Department), new { companyId = model.CompanyFK });
        }
        ...
        //    vmCommonCustomer.MonthlyIncentive = "As per company policy";
        //    vmCommonCustomer.YearlyIncentive = "As per company policy";
