...

{
    ...
    t2.VoucherDate >= fromExistDate
        ...
        public object CommonGetAutoCompleteVendorHeadGL(string prefix, int companyId, int vendorTypeId)
        {
            //var userId = Common.GetIntUserId();
            //if (userId <= 0)
            //{
    ...
    t2.VoucherDate >= fromExistDate
        ...
        public async Task<long> AccountingSalesReturnPushSeed(int CompanyFK, VMSaleReturnDetail vmSaleReturnDetail, int journalType)
        {
            ...
            double Disc = unitDiscount + spetialDiscount;

            double GrossSales = vmSaleReturnDetail.DataListDetail.Any()
                ? Convert.ToDouble(vmSaleReturnDetail.DataListDetail.Sum(x => Convert.ToDouble(x.Qty.Value * x.Rate.Value)))
                : 0;
            ...
            if (Math.Round(totalDebit, 2) != Math.Round(totalCredit, 2))
                throw new Exception($"Sales Return Journal not balanced. Debit={totalDebit}, Credit={totalCredit}, SaleReturnNo={vmSaleReturnDetail.SaleReturnNo}");
