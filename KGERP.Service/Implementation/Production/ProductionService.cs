...
namespace KGERP.Services.Production
{
    public class ProductionService
    {
        ...
        public async Task<int> ProdReferenceSlaveByChallanAdd(VMProdReferenceSlave vmProdReferenceSlave)
        {
            ...
            foreach (var item in DataListSlavePartial)
            {
                ...
                if (result > 0 && vmProdReferenceSlave.CompanyFK == (int)CompanyName.KrishibidSeedLimited)
                {
                    ...
                    await _accountingService.AccountingJournalPush(prodReference.ReferenceDate, vmProdReferenceSlave.CompanyFK.Value, drAccountHead, crAccountHead, amount, title, description, (int)JournalEnum.JournalVoucer); //Raw Materials 30803 Paddy seeds
        ...
        public List<object> GCCLLCFactoryExpanceHeadGLList(int companyId)
        {
            ...
            foreach (var item in _db.HeadGLs.Where(x => x.CompanyId == companyId && (x.ParentId == 39387 || x.ParentId == 50613100)).ToList())
            {
